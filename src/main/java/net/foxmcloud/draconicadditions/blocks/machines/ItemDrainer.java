...
public class ItemDrainer extends BlockBCore implements ITileEntityProvider {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		TileItemDrainer tileItemDrainer = worldIn.getTileEntity(pos) instanceof TileItemDrainer ? (TileItemDrainer) worldIn.getTileEntity(pos) : null;
		return state.withProperty(ACTIVE, tileItemDrainer != null && tileItemDrainer.active.value);
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, DraconicAdditions.instance, GUIHandler.GUIID_ITEM_DRAINER, world, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if (stateIn.getActualState(worldIn, pos).getValue(ACTIVE)) {
			...
			switch (enumfacing) {
			case WEST:
				worldIn.spawnParticle(EnumParticleTypes.END_ROD, d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D, new int[0]);
			...
			case EAST:
				...
				break;
			case NORTH:
				worldIn.spawnParticle(EnumParticleTypes.END_ROD, d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D, new int[0]);
